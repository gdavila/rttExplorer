...
plot_rtt <- function(rtt, probe_ttl){
  ...
  #subset
  #print('subset')
  #rtt1 <- subset(rtt, hops.tx.sec - min(hops.tx.sec) <= 17000 )
  #rtt2 <- subset(rtt, hops.tx.sec - min(hops.tx.sec) > 17000 )
  ...
  p1 <- ggplot(data=rtt_density(sort(rtt$rtt)) , aes(x=mids, y=counts))+
   ...
   #p1 <- ggplot()+
  #      geom_line(data=rtt_density(sort(rtt1$hops.rtt)) , aes(x=mids, y=counts), color="blue")+
  #      geom_line(data=rtt_density(sort(rtt2$hops.rtt)) , aes(x=mids, y=counts), color="red")+
  #      scale_y_continuous(name = 'density', trans = log_trans(), breaks = base_breaks()) + 
  ...
  p2 <- ggplot(data=rtt, aes(x=tx.sec - min(tx.sec), y=rtt))+
        geom_point(color="gray", alpha=0.3) +
        scale_y_continuous(limits=c(min(rtt$rtt),quantile(rtt$rtt, .98)))+
        theme_gray()+
...
q <- paste('{ "src" : ', ip_src, 
           ...
           ', "$lt" : ', finish, '}',
           '}')
f <- '{ "hops.rtt" : true, "hops.tx.sec":true,  "hops.addr" : true , "hops.reply_ttl" : true ,  "_id" : false}'
s <- '{"hops.rtt" : 1}'
