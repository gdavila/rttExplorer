...
get_pathStability <- function(paths){
  ...
  for (i in 1: nrow(paths$start)){
    ...
    if (i==1){
      ...
      dateStart <- append(dateStart,paths$date[i])
      ...
      start <- append(start,paths$start[['sec']][i])
      dateStart <- append(dateStart,paths$date[i])
      dateFinish <- append(dateFinish,paths$date[i])
      changing_paths <- append(changing_paths,list(paths$Hops[[i]]['from']$from))
      probe_ttl <-append(probe_ttl,list(paths$Hops[[i]]['hop']$hop))
      mpls_info <- append(mpls_info, get_mplsInfo(paths$Hops[[i]][['ICMPExtensions']],paths$Hops[[i]][['hop']]))
      mb_modifications <- append(mb_modifications, get_mbModifications(paths$Hops[[i]][['Modifications']])) 
  ...
  paths_stability_df$dateStart <- as.POSIXlt(strptime(dateStart, '%m/%d/%y %H:%M:%S'))- 3600*3
  paths_stability_df$dateFinish <- as.POSIXlt(strptime(dateFinish, '%m/%d/%y %H:%M:%S')) - 3600*3
  #paths_stability_df$mbAdditions <- mb_additions
  #paths_stability_df$mbDeletions <- mb_deletions
  return(paths_stability_df)
}

plot_comparision_rtt <- function(RTT, date_str){
  ...
  rtt$ftime <- as.POSIXlt(strptime(RTT$start$ftime, '%Y-%m-%d %H:%M:%S')) - 3600*3
  ...
  rtt1 <- subset(rtt, ftime <= as.POSIXlt(strptime(date_str, "%Y-%m-%d %H:%M:%S")))
  rtt2 <- subset(rtt, ftime > as.POSIXlt(strptime(date_str, "%Y-%m-%d %H:%M:%S")))
  #View(rtt1)
  ...
  p2 <- ggplot()+
  geom_point(data= subset (rtt1, rtt <= quantile(rtt, .99)), aes(x=ftime, y=rtt), color="blue", alpha=0.3) +
  geom_point(data= subset (rtt2, rtt <= quantile(rtt, .99)), aes(x=ftime, y=rtt), color="red", alpha=0.3) + 
  scale_x_datetime()+
...
```{r, echo =FALSE, results=FALSE, message=FALSE, warning=FALSE}
plot_comparision_rtt2 <- function(RTT, date_str1, date_str2 ){
  rtt <-sapply(RTT$hops, unlist)
  rtt <- as.data.frame(t(rtt),stringsAsFactors = FALSE)
  #rtt <-  t(apply(rtt, 1, unlist))
  #rtt <-  as.data.frame(rtt,stringsAsFactors = FALSE)
  ...
  rtt1 <- subset(rtt, ftime <= as.POSIXlt(strptime(date_str1, "%Y-%m-%d %H:%M:%S")))
  rtt2 <- subset(rtt, ftime > as.POSIXlt(strptime(date_str1, "%Y-%m-%d %H:%M:%S")) & ftime < as.POSIXlt(strptime(date_str2, "%Y-%m-%d %H:%M:%S")))
  rtt3 <- subset(rtt, ftime > as.POSIXlt(strptime(date_str2, "%Y-%m-%d %H:%M:%S")))
  ...
  geom_line(data=rtt_density(sort(rtt1$rtt)) , aes(x=mids, y=counts), color="blue")+
  geom_line(data=rtt_density(sort(rtt2$rtt)) , aes(x=mids, y=counts), color="red")+
  geom_line(data=rtt_density(sort(rtt3$rtt)) , aes(x=mids, y=counts), color="orange")+  
  scale_y_continuous(name = 'density', trans = log_trans(), breaks = base_breaks()) + 
  scale_x_continuous(name = 'rtt (s)', trans = log_trans(), breaks = base_breaks()) +
  theme_gray()+
  theme(axis.text.x = element_text(angle = 90))
  ...
  geom_point(data= subset (rtt1, rtt <= quantile(rtt, .99)), aes(x=ftime, y=rtt), color="blue", alpha=0.3) +
  geom_point(data= subset (rtt2, rtt <= quantile(rtt, .99)), aes(x=ftime, y=rtt), color="red", alpha=0.3) + 
  geom_point(data= subset (rtt3, rtt <= quantile(rtt, .99)), aes(x=ftime, y=rtt), color="orange", alpha=0.3) + 
  scale_x_datetime()+
  scale_y_continuous(limits=c(min(rtt$rtt),quantile(rtt$rtt, .99)))+
  theme_gray()+
  labs( x = "time (s)", y = "rtt (s)")
  ...
  geom_line(data= rtt1, aes(x=ftime, y=reply_ttl), color="blue") +
  geom_line(data= rtt2, aes(x=ftime, y=reply_ttl), color="red") + 
  geom_line(data= rtt3, aes(x=ftime, y=reply_ttl), color="orange") + 
  scale_x_datetime()+
  theme_gray()+
  labs( x = "time (s)", y = "reply ttl")
  #ggtitle(paste0(ip,"\n probe ttl = ", probe_ttl)) 
  ...
  geom_line(data= rtt1, aes(x=ftime, y=probe_ttl), color="blue") +
  geom_line(data= rtt2, aes(x=ftime, y=probe_ttl), color="red") + 
  geom_line(data= rtt3, aes(x=ftime, y=probe_ttl), color="orange") + 
  scale_x_datetime()+
  theme_gray()+
  labs( x = "time (s)", y = "probe ttl")
  #ggtitle(paste0(ip,"\n probe ttl = ", probe_ttl)) 
...
* DateStart: Fecha y Hora en que el path (constante) fue descubierto por primera vez
* DateStart: Fecha y hora en la que finaliza el Path
* Duration: Duración del Path Estable en segundos

```{r, echo =FALSE, results=TRUE, message=FALSE, warning=FALSE, rows.print=15}
...
pathStability[, c('dateStart','dateFinish','duration')]
```
...
for (path in 1:nrow(pathStability)){
  print(data.frame(sapply(pathStability[path, c('probe_ttl', 'hops', 'mplsInfo')],unlist)))
...
#print("rtt")
if(nrow(rtt) != 0){
  print(plot_comparision_rtt(rtt, "2018-05-03 03:59:56") )
}
```
...
if(nrow(rtt) != 0){
  print(plot_comparision_rtt(rtt, "2018-05-03 03:59:56") )
  print(plot_comparision_rtt(rtt, "2018-05-03 01:09:40") )
}
```
...
if(nrow(rtt) != 0){
  print(plot_comparision_rtt(rtt, "2018-05-03 03:59:56") )
  print(plot_comparision_rtt2(rtt, "2018-05-03 01:09:40", "2018-05-03 02:14:49") )
